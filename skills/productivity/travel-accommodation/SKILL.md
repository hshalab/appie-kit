...
def ddg_search(query):
    ...
    # decode URLs & titles
    return list(zip(titles, results))  # approx
```
...
**Keywords to match:** `hotel`, `resort`, `villa`, `kids`, `family`, `club`, `pool`, `suite`, `luxury`, `price`, `rate`, `beach`, `restaurant`, `spa`
...
Direct Airbnb URL for area overview:
...
Always end with a top pick recommendation and why. If the user asks for alternatives, suggest 1-2 contrasting options (e.g. one ultra-luxury, one better value).
...
- **Travel/booking sites block headless browsers** (TripAdvisor, Booking.com, airline sites) — use curl with real browser User-Agent as fallback, or search via DuckDuckGo HTML endpoint instead
- **Google, Bing, TripAdvisor all CAPTCHA** from headless — DDG HTML endpoint is the most reliable for search results
- **macOS grep lacks -P flag** — use Python `re.findall()` for regex extraction instead
- **Airbnb listing pages are JavaScript-heavy** — the initial HTML may not contain full listing data; use Airbnb search URLs and parse JSON-LD if present
- **Price ranges are seasonal** — mention that rates vary by season (high season June-Aug, Dec-Jan; low season Feb-May, Sep-Nov for Bali)
- **"Family-friendly" means different things** — a baby needs different amenities than a 10yo. Always match to the specific family composition (baby, toddler, teen)
- **Less busy != no amenities** — quieter areas may have fewer restaurants/shops. Flag trade-offs
- **Article data can be stale** — check publication dates on articles; prices and ratings change
- **DuckDuckGo HTML endpoint may also CAPTCHA** after many requests — space queries 2-3 seconds apart via time.sleep()
- **Airbnb direct scraping is fragile** — prefer article reviews that reference specific Airbnb listings

## Reference Files

- `references/bali-uluwater-family-research-2026-06.md` — compiled research from a session on family-friendly luxury resorts and Airbnbs in Uluwatu, Bali for a client (Amir). Includes hotel + Airbnb options with ratings, price ranges, and booking links.