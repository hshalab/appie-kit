...
If `agent-browser install` fails on ARM64 (e.g., on a Hetzner ARM64 VPS) with an error like "Chrome for Testing does not provide Linux ARM64 builds," you need to install Chromium from your system's package manager and then configure `agent-browser` to use it.

1.  **Install Chromium:**
    ```bash
    sudo apt update && sudo apt install -y chromium-browser
    ```
    (Note: If `apt` reports a locked cache, run `sudo dpkg --configure -a` first to resolve it.)

2.  **Configure agent-browser:**
    ```bash
    agent-browser --executable-path /usr/bin/chromium
    ```

    Once configured, you should be able to use the `browser_navigate` tool and other `browser-use` functionalities.
