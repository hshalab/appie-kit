...
AI mode uses OpenClaw's OpenAI-compatible endpoint (`/v1/chat/completions`). This is **disabled by default** for security. To enable:
...
- The script reads the token from `~/.openclaw/openclaw.json` automatically
...
- The hook requires OpenClaw to be running for AI summaries
- Without OpenClaw, falls back to raw git info mode
