...
- [`n8n-agents`](n8n-workflows/n8n-agents/): Design n8n AI agents the right way. Covers Agent-vs-chain-vs-classifier, model/memory/tools/outputParser slots, tool descriptions as prompt, structured output, RAG, and human review.
- [`n8n-binary-and-data`](n8n-workflows/n8n-binary-and-data/): Handle files and binary data in n8n correctly. Covers $binary vs $json split, reading/writing binary, Merge node binary preservation, agent-tool binary boundary, and CDN/URL requirement for chat surfaces.
...
- [`n8n-code-tool`](n8n-workflows/n8n-code-tool/): Write JavaScript or Python for the n8n Custom Code Tool — the AI-agent-callable tool. Covers critical differences between Code node and Code Tool.
- [`n8n-error-handling`](n8n-workflows/n8n-error-handling/): Wire n8n error handling so failures are loud, structured, and recoverable. Covers per-node error outputs, retry/self-healing, error-trigger workflows, and 4xx/5xx response shapes.
...
- [`n8n-multi-instance`](n8n-workflows/n8n-multi-instance/): Use when working with multiple n8n instances over MCP — choosing, switching, verifying targets, and recovering from INSTANCE_AMBIGUOUS errors.
...
- [`n8n-self-hosting`](n8n-workflows/n8n-self-hosting/): Deploy a production self-hosted n8n to a fresh Linux VM using Docker Compose + Caddy reverse proxy with automatic HTTPS. Covers single/queue modes.
- [`n8n-subworkflows`](n8n-workflows/n8n-subworkflows/): Build reusable, composable n8n sub-workflows. Covers typed inputs, all-vs-each execution, verb-first naming, and splitting by input shape.
